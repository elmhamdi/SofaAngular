...
export class ChatService {
   ...
   */
  private initial() {
    this.getResponse('مرحبا').subscribe((r: any) => {
      this.addMessageToChat({ text: r.result.fulfillment.speech, bot: true });
      this.possibleAnswers.next(['نعم', 'لا', 'من?']);
    });
  }
  ...
  checkAction(actionType: string, params: any) {
    switch (actionType) {
      // search locations
      case 'search.places': {
        const addressSuggestions = ['تحديد مكاني'];
      ...
      case 'add.location.to.favorites': {
        ...
        this.possibleAnswers.next(['إظهار المفضلة']);
        break;
      }
      ...
      case 'show.favorites': {
        ...
        this.possibleAnswers.next(['ابحث عن بدائل']);
        break;
      }
  ...
  private handleGetLocations(res: any) {
    ...
    if (this.locationsList.length > 0) {
      this.addMessageToChat({
        text: 'هذه أماكن مناسبة:',
        bot: true,
        locationsList: locationsTrimmed
      });
      this.possibleAnswers.next(['عرض المزيد', 'شكرا جزيلا']);
      this.listStartIndex += locationsTrimmed.length;
    } else {
      this.addMessageToChat({
        text: 'للأسف لم أتمكن من العثور على أي مكان 😕',
        bot: true,
      });
      this.possibleAnswers.next(['افعل شيئا اخر ']);
    }
  }
